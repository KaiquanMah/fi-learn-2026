  ...
  return (
    <div className="container mx-auto p-4 max-w-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(Object.keys(UserType) as Array<keyof typeof UserType>).map((key) => (
            <button
              ...
              className={`p-4 rounded-lg border-2 text-left transition-all ${settings.userType === UserType[key]
      ...
      <div className="mt-8 text-center text-gray-400 text-sm">
        <label className="block mb-2 font-medium text-gray-700 dark:text-gray-300">Gemini API Key</label>
        <input
          type="password"
          value={settings.apiKey || ''}
          onChange={(e) => updateSettings({ apiKey: e.target.value })}
          placeholder="Enter your Gemini API Key"
          className="p-2 border rounded w-full max-w-md bg-white dark:bg-slate-700 dark:text-white"
        />
        <p className="mt-2 text-xs">Key is stored locally in your browser.</p>
      </div>
    </div>
  );
};